...
RSpec.describe 'notify me', type: :feature do
  ...
  context 'when a movie is hated' do
    subject { NotifyMeMailer.movie_hated(author.email) }

    it 'should be delivered to the author email when there is a hate' do
      is_expected.to deliver_to(author.email)
    end

    it { is_expected.to have_subject(/Somebody has hated your movie/) }
  end
end
